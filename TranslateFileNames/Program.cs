...
namespace TranslateFileNames
{
    public class Program
    {
        ...
        public static void Main(string[] args)
        {
            ...
            VerboseOutput("Scanning path \"" + _path + "\"" + RecursiveModeText + " for files to translate.\n" +
            ...
            {
                VerboseOutput("Parsing file: " + file);
                ...
                {
                    VerboseOutput("Error: Failed to get file name for  \"" + file + "\".", OutPutLevel.ErrorLvl);
                    continue;
                }
                    ...
                    (sourceText.Length + fileDetails.Name.Length + 4 > _maximumTranslateDataLength))
                {
                    ...
                    while (Volatile.Read(ref _workerThreadCount) > _maxWorkerThreads)
                        Thread.Sleep(10);
                    VerboseOutput("** Sending " + subCount.ToString() + " file names to translator.");
            ...
            if (_renameErrorCount > 0)
                VerboseOutput(_renameErrorCount + " Rename errors occurred.", OutPutLevel.PostProgressBar);
            if (_errorCount > 0)
                VerboseOutput(_errorCount + " unknown errors occurred.", OutPutLevel.PostProgressBar);
            VerboseOutput("Translation completed. Elapse time = " + sw.Elapsed, OutPutLevel.PostProgressBar);
            if (_waitKeyPress)
            {
        ...
        public static bool Initialize(string[] args)
        {
            ...
            ParseArgs(args);
            VerboseOutput("Opening path: " + _path, OutPutLevel.VerboseIfNotSilent);
            if (!Directory.Exists(_path))
            {
                VerboseOutput("Path does not exist. Exiting.", OutPutLevel.ErrorLvl);
                return false;
            }
            ...
            if (_createUndoList)
            {
                ...
                {
                    VerboseOutput("Error: Early exit because could not create undo file at path \"" + _undoListFileName + "\".\nErr Msg: \"" + e.Message + "\"", OutPutLevel.ErrorLvl);
                    ++_errorCount;
                    return false;
                }
            }
            ...
            {
                VerboseOutput("No files exist. Exiting.");
                VerboseOutput("Warning: Early exit because no files exist in path \"" + _path + "\".", OutPutLevel.ErrorLvl);
                return false;
            }
        ...
        private static async Task TranslateSourceText(string sourceText, List<FileDetails> fileDetailsList)
        {
            try
            {
                try
                {
                    ...
                    VerboseOutput(ee.Message, OutPutLevel.ErrorLvl);
                    ++_errorCount;
                ...
                foreach (FileDetails fileDetail in fileDetailsList)
                {
                    ...
                    { 
                        ...
                        if (fileDetail.Translation.Equals(fileDetail.Name))
                            VerboseOutput("Skipping file rename \"" + fileDetail.Name + "\" to \"" + fileDetail.Translation + "\"");
                        else
                        {
                            ++_renameCount;
                            if (_noRename)
                            {
                                ...
                                VerboseOutput("Rename candidate \"" + oldFileName + "\" to-> \"" + newFileName + "\"");
                            }
                            else
                            {
                                try
                                {
                                    ...
                                    VerboseOutput("Renamed file \"" + oldFileName + "\" to \"" + newFileName + "\"");
                                } catch (Exception exc) {
                                    ...
                                    if (newIndexFileName != null)
                                    {
                                        ...
                                        VerboseOutput("Renamed file \"" + oldFileName + "\" to \"" + newIndexFileName + "\"");
                                    }
                                    else
                                    {
                                        --_renameCount;
                                        ++_renameErrorCount;
                                        VerboseOutput("Error: Could not rename file \"" + oldFileName + "\" to \"" + newFileName + "\"\nErr Msg: " + exc.Message, OutPutLevel.ErrorLvl);
                                    }
                                }
                            }
                        }
                    } catch (Exception eeeee) {
                        VerboseOutput(eeeee.Message, OutPutLevel.ErrorLvl);
                        ++_errorCount;
                ...
                if (UndoListText.Length > 0)
                {
                    if (_createUndoList)
                    {
                        VerboseOutput("Creating undo list file at \"" + _undoListFileName + "\"");
                        Monitor.Enter(_lock);
                        try
                        {
                 ...
                 VerboseOutput(e.Message, OutPutLevel.ErrorLvl);
                ++_errorCount;
            }
        }
        ...
        private static string RenameFileWithAppendedIndex(string oldFileName, string newFileName, string Ext)
        {
            for (int i = 0; i < 199; ++i)
            {
                ...
                {
                    ++_renameErrorCount;
                    VerboseOutput("Error: Could not rename file \"" + oldFileName + "\" to \"" + newIndexFileName + "\"\nErr Msg: " + e.Message, OutPutLevel.ErrorLvl);
                }
            }
            return null;
        }
        ...
        private static async Task GTranslate_TranslateSourceText(string sourceText, List<FileDetails> fileDetailsList)
        {
            ...
            try
            {
                ...
                VerboseOutput($"Translated text: {result.Translation}");
                VerboseOutput($"Source Language: {result.SourceLanguage}");
                VerboseOutput($"Target Language: {result.TargetLanguage}");
                VerboseOutput($"Service: {result.Service}");
                string SrcLang = "";
        ...
        private static void VerboseOutput(string message, OutPutLevel outputlevel = OutPutLevel.NormalLvl)
        { 
            if (_silent)
                return;
            else if (outputlevel == OutPutLevel.ErrorLvl || outputlevel == OutPutLevel.PreProgressBar || outputlevel == OutPutLevel.PostProgressBar)
            {
                ...
                Console.Write(".");
            else if (_verbose || outputlevel == OutPutLevel.VerboseIfNotSilent)
                Console.WriteLine(message);
        }
